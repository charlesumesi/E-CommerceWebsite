...
def productToHire(request, product_to_hire_id):
    ...
    if request.method == 'POST' and Hire_before_rate.objects.using('default').filter(product=product_to_hire, user=request.user).exists():
        ...
        if rate_product == "1":
            save_reviewh = Customer_ph_rating(name=request.user.email.split('@')[0].capitalize(), email=request.user,
    ...
    elif request.method == 'POST' and Hire_before_rate.objects.using('default').filter(product=product_to_hire, user=request.user).exists() == False:
...
def productToBuy(request, product_to_buy_id):
    ...
    if request.method == 'POST' and Buy_before_rate.objects.using('default').filter(product=product_to_buy, user=request.user).exists():
        product_name = request.POST['product_name']
        product_code = request.POST['product_code']
        rate_product = request.POST['rate_product']
        review_title = request.POST['review_title']
        comment = request.POST['comment']
        if rate_product == "1":
            save_reviewb = Customer_pb_rating(name=request.user.email.split('@')[0].capitalize(), email=request.user,
    ...
    elif request.method == 'POST' and Buy_before_rate.objects.using('default').filter(product=product_to_buy, user=request.user).exists() == False:
...
def checkout(request):
    ...
    customers = Customer.objects.using('default')
    shipping_addresses = Shipping_address.objects.using('default')
...
def payment(request):
    ...
    customers = Customer.objects.using('default')
    shipping_addresses = Shipping_address.objects.using('default')
    country_list_copy1 = ['UK', 'UNITED KINGDOM', 'GREAT BRITAIN', 'BRITAIN', 'GB', 'GBR', 'ENGLAND', 'SCOTLAND', 'WALES', 'NORTHERN IRELAND', 'N/IRELAND']
    ...
    context = {
        ...
        'country_list_copy1' : country_list_copy1,
    }
...
def transactionComplete(request):
    ...
    payment_cards = Payment_card.objects.using('default')
    ...
    if request.COOKIES["order_cookie_" + request.session.session_key + "_" + str(request.session['visit_no'] - 1)]:
        ...
        country_list_copy2 = ['UK', 'UNITED KINGDOM', 'GREAT BRITAIN', 'BRITAIN', 'GB', 'GBR', 'ENGLAND', 'SCOTLAND', 'WALES', 'NORTHERN IRELAND', 'N/IRELAND']
        code_dict = {}
        ...
        for payment_card in payment_cards:
            if payment_card.transaction_key == request.session.session_key + "_" + str(request.session['visit_no'] - 1) and payment_card.country.upper() in country_list_copy2:
                grand_total = eval(total) + (17.5/100 * eval(total)) + 50
                ...
                payment_card.save()
            elif payment_card.transaction_key == request.session.session_key + "_" + str(request.session['visit_no'] - 1) and payment_card.country.upper() not in country_list_copy2:
                grand_total = eval(total) + (17.5/100 * eval(total)) + 200
                save_total = Total(user=request.user, name=payment_card.name_on_card,
                total=grand_total, timestamp=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                transaction_key=request.session.session_key + "_" + str(request.session['visit_no'] - 1))
                save_total.save()
